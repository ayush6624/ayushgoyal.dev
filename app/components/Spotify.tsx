        ...
        >
          <p className="font-medium">
            {music?.status === true ? (
              ...
              <p>
                Psst. Nothing is playing right now
                {showRedirect && (
                  <>
                    , but maybe checkout{' '}
                    <Link
                      to="/music"
                      className="text-blue-400 underline decorated-dotted"
                    >
                      /music
                    </Link>
                    ?
...
export const RecentTracks = ({ recentTracks }: { recentTracks?: any }) => {
  if (!recentTracks || recentTracks?.status === false) return <>Loading...</>;

  return (
    <div>
      <ul className="list-disc">
        {recentTracks?.data.slice(0, 3).map((track: any) => (
          <li
            className="my-1 text-gray-500 truncate max-w-max"
            key={track.name}
          >
            <span className="text-black">{track.name}</span> - {track.artist}
          </li>
        ))}
      </ul>
    </div>
